...
int main(int argc, char **argv) {
    ...
    cliInput.addArgument(InputArgument("bands", ArgumentType::Integer, "Number of bands of spherical harmonics", false, "2"));
...
//    }
    ...
    try {
        ...
        const int bands = arguments["bands"].value.asInteger;
        ...
        cout << px << endl << nz << endl;
        cout << bands << endl << samples << endl << arguments["filtering"].value.asString << endl;
        cout << arguments["px"].value.asString << endl;

        auto cubeMap = loadCubemapRgbFloat(px, nx, py, ny, pz, nz);
        CubeMapPolarFunction<RGBFloat> polarFunction(cubeMap, filtering);

        ShCoefficients<RGBFloat, 2> shCoefficients = encode<RGBFloat, 2>(polarFunction, method, (uint16_t) samples);

        write<RGBFloat, 2>(output, shCoefficients);
    }
